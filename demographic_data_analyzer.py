...
def calculate_demographic_data(print_data=True):
    ...
    lower_education_rich = lower_education_rich = round(df[~df['education'].isin(['Bachelors', 'Masters', 'Doctorate'])]['salary'].value_counts(normalize=True)['>50K'] * 100, 1)
    ...
    rich_percentage = round((num_min_workers['salary'].value_counts(normalize=True).get('>50K', 0)) * 100, 1)
    ...
    highest_earning_country_percentage = round((df.groupby('native-country')['salary'].value_counts(normalize=True).unstack().fillna(0)['>50K'].max() * 100), 1)
